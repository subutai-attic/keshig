...
public class KeshigRestServiceImpl implements KeshigRestService {
    ...
    public Response addServer(Server server) {
        ...
        if (server == null) {
            Response response = Response.status(Response.Status.BAD_REQUEST).entity("Invalid server").build();
            return response;
        }
        ...
        try {
            ...
            LOG.error("Error saving new server ", server.toString());
            Response response = Response.status(Response.Status.BAD_REQUEST).entity("Invalid server").build();
            return response;
    ...
    public Response updateServer(Server server) {
        try {
            ...
            keshig.updateServer(server);
            return Response.ok().build();

        } catch (Exception e) {

            LOG.error("Error saving new server ", server.toString());
            Response response = Response.status(Response.Status.BAD_REQUEST).entity("Invalid server info for update").build();
            return response;
